 ...
 * Scrolls horizontally on narrow viewports instead of clipping personas
 * that don't fit (mx-auto on the inner w-max row keeps it centered when
 * it fits, without the flex `justify-center` scroll-clipping bug).
 */
export default function PersonaSelector({ selectedPersona, onSelect }: PersonaSelectorProps) {
    return (
        <div className="w-full overflow-x-auto scrollbar-hide pointer-events-auto">
            <div className="flex items-center gap-2 md:gap-10 px-4 md:px-8 py-4 w-max mx-auto bg-[#0A0908] border border-[#E3D5CA]/5 rounded-3xl shadow-2xl backdrop-blur-xl">
